...
- **Controle otimista de concorrência**: `Project` e `Task` ganharam uma coluna `version` (inteiro, começa em `1`). `PUT /projects/{id}` e `PATCH /tasks/{id}` agora exigem um campo `version` no corpo da requisição — o valor que o cliente viu na última leitura (`GET`). Antes de aplicar qualquer mudança, a rota compara esse valor com o `version` atual no banco: se não bater, rejeita com `409 VERSION_CONFLICT` sem tocar no registro (a atualização de outra requisição, feita entre o `GET` e o `PUT`/`PATCH` do cliente, não é sobrescrita silenciosamente — modelo "last-write-wins" evitado). Se bater, aplica os campos alterados e incrementa `version` em `1`, dentro da mesma transação do `commit()`. Preferimos uma coluna `version` explícita e comparação manual em vez do mecanismo nativo do SQLAlchemy (`__mapper_args__ = {"version_id_col": ...}`, que compara a versão do objeto carregado contra o banco automaticamente a cada flush): o mecanismo nativo é pensado para um objeto que nunca saiu da sessão, não para o caso de API REST onde o cliente informa, via payload, a versão que ele acredita ser a atual — exigiria sobrescrever manualmente o atributo `version` do objeto carregado com o valor do payload antes do commit, o que é mais confuso de ler e testar do que a comparação explícita. Testado em `tests/test_projects.py` e `tests/test_tasks.py`: uma atualização bem-sucedida incrementa a versão, e uma segunda atualização reaproveitando a versão antiga recebe `409`.

Todos os diferenciais opcionais listados no enunciado foram implementados.
